...
const FIREBASE_CONFIG = {
  apiKey: "AIzaSyCWuU09hd_Hp0aYOMQhAMd1ln5Ow5iRuGI",
  authDomain: "viber5-4874f.firebaseapp.com",
  projectId: "viber5-4874f",
  storageBucket: "viber5-4874f.firebasestorage.app",
  messagingSenderId: "325850809844",
  appId: "1:325850809844:web:78f92b990bacec777265a1",
};
...
let pendingSignup = null; // stores {name, email, password} while awaiting verification
...
function initFirebase() {
  try {
    ...
    firebaseAuth.onAuthStateChanged(user => {
      if (user && user.emailVerified) {
        ...
        document.getElementById('verify-subtitle').textContent =
          `We sent a verification email to ${user.email}. Check your inbox.`;
      } else {
        showAuthWrap();
      }
    });
    ...
    // Fallback: skip auth if Firebase not configured
    if (FIREBASE_CONFIG.apiKey === 'REPLACE_WITH_YOUR_FIREBASE_API_KEY') {
      showAuthWrap();
    }
  }
}
...
function enterApp(user) {
  ...
  // Optionally show user name somewhere
}
...
document.getElementById('signup-btn').addEventListener('click', async () => {
  ...
  if (!firebaseAuth || FIREBASE_CONFIG.apiKey === 'REPLACE_WITH_YOUR_FIREBASE_API_KEY') {
    return showError('signup-error', 'Firebase is not configured yet. See setup instructions.');
  }
  ...
  try {
    ...
    document.getElementById('verify-subtitle').textContent =
      `We sent a 6-digit verification link to ${email}. Check your inbox and click the link.`;
    showAuthPage('auth-verify');
  } catch (e) {
    const msgs = {
...
document.getElementById('signin-btn').addEventListener('click', async () => {
  ...
  if (!firebaseAuth || FIREBASE_CONFIG.apiKey === 'REPLACE_WITH_YOUR_FIREBASE_API_KEY') {
    return showError('signin-error', 'Firebase is not configured yet. See setup instructions.');
  }
  ...
  try {
    ...
    if (!cred.user.emailVerified) {
      await cred.user.sendEmailVerification();
      document.getElementById('verify-subtitle').textContent =
        `Your email is not verified. We resent the link to ${email}.`;
      showAuthPage('auth-verify');
    }
    ...
    showError('signin-error', msgs[e.code] || 'Sign in failed. Please try again.');
  } finally {
...
document.getElementById('resend-code-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    showSuccess('verify-success', 'Verification email resent! Check your inbox.');
  } catch (e) {
    showError('verify-error', 'Could not resend. Try again in a minute.');
  }
});

// ── Verify: poll for email verification ──
document.getElementById('verify-btn').addEventListener('click', async () => {
  const user = firebaseAuth?.currentUser;
  if (!user) return showError('verify-error', 'No active session. Please sign up again.');
  ...
  try {
    ...
    if (user.emailVerified) {
      ...
      showError('verify-error', 'Email not verified yet. Click the link in your email first, then tap this button.');
    }
  } catch (e) {
    showError('verify-error', 'Check failed. Try again.');
  } finally {
    btn.disabled = false; btn.textContent = 'I verified my email ✓';
  }
});

// Change verify button text to be clearer
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('verify-btn').textContent = 'I verified my email ✓';
});

// 6-digit code input: auto-advance between boxes (used as a visual element)
document.querySelectorAll('.code-digit').forEach((input, i, inputs) => {
...
async function signInWithGoogle() {
  if (!firebaseAuth || FIREBASE_CONFIG.apiKey === 'REPLACE_WITH_YOUR_FIREBASE_API_KEY') {
    return showError('signin-error', 'Firebase is not configured yet.');
  }
  try {
    ...
    showError('signin-error', msgs[e.code] || 'Google sign-in failed. Try again.');
  }
}
